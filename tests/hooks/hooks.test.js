...
async function runTests() {
  ...
  if (await asyncTest('creates or updates session file', async () => {
    ...
    // セッションファイルが作成されたかチェック
    // 注意: CLAUDE_SESSION_IDがない場合、プロジェクト名にフォールバック（'default'ではない）
    // スクリプトのgetDateString()関数に合わせてローカル時刻を使用
    const sessionsDir = path.join(os.homedir(), '.claude', 'sessions');
    const now = new Date();
    const today = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;

    // 期待されるセッションIDを取得（プロジェクト名フォールバック）
  ...
  if (await asyncTest('includes session ID in filename', async () => {
    ...
    const expectedShortId = 'abc12345'; // 最後の8文字
    ...
    // セッションIDを含むセッションファイルが作成されたかチェック
    // スクリプトのgetDateString()関数に合わせてローカル時刻を使用
    const sessionsDir = path.join(os.homedir(), '.claude', 'sessions');
    const now = new Date();
    const today = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
  ...
  if (await asyncTest('processes sessions with enough messages', async () => {
    ...
    // より長いトランスクリプトを作成（10を超えるユーザーメッセージ）
  ...
  if (test('hooks.json is valid JSON', () => {
    ...
    JSON.parse(content); // 不正な場合は例外をスロー
  })) passed++; else failed++;
      ...
      for (const entry of hookArray) {
        for (const hook of entry.hooks) {
          if (hook.type === 'command' && hook.command.includes('scripts/hooks/')) {
            // コマンド内のリテラル文字列 "${CLAUDE_PLUGIN_ROOT}" をチェック
  ...
  if (test('plugin.json does NOT have explicit hooks declaration', () => {
    // Claude Codeは規約により hooks/hooks.json を自動的に読み込みます。
    // plugin.jsonで明示的に宣言すると重複検出エラーが発生します。
