| name | description |
|------|-------------|
| cloud-infrastructure-security | クラウドプラットフォームへのデプロイ、インフラ設定、IAMポリシー管理、ログ/モニタリング設定、CI/CDパイプライン実装時に使用。ベストプラクティスに沿ったクラウドセキュリティチェックリストを提供。 |

# クラウドとインフラセキュリティスキル

このスキルは、クラウドインフラ、CI/CDパイプライン、デプロイメント設定がセキュリティベストプラクティスに従い、業界標準に準拠することを保証します。
...
resource "aws_db_instance" "main" {
  ...
  backup_retention_period = 30  # 30日保持
...
- [ ] **IAM**: rootアカウント未使用、MFA有効化、最小権限ポリシー
- [ ] **シークレット**: すべてのシークレットをクラウドシークレットマネージャーにローテーション付きで
- [ ] **ネットワーク**: セキュリティグループ制限、公開データベースなし
- [ ] **ログ記録**: CloudWatch/ログを保持付きで有効化
- [ ] **モニタリング**: 異常のアラートを設定
...
- [ ] **ドキュメント**: インフラを文書化、ランブックを作成
- [ ] **インシデントレスポンス**: セキュリティインシデント計画を実施
...
**覚えておくこと**: クラウド設定ミスはデータ侵害の主な原因です。単一の露出されたS3バケットまたは過度に寛容なIAMポリシーがインフラ全体を危険にさらす可能性があります。常に最小権限と多層防御の原則に従ってください。
