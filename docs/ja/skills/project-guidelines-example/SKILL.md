...
これはプロジェクト固有のスキルの例です。独自のプロジェクトのテンプレートとして使用してください。

実際の本番アプリケーションに基づく: [Zenith](https://zenith.chat) - AI駆動の顧客発見プラットフォーム。

---

## 使用タイミング

このスキルが設計された特定のプロジェクトに取り組む際に参照してください。プロジェクトスキルには以下が含まれます:
- アーキテクチャ概要
- ファイル構造
- コードパターン
- テスト要件
...
3. **TDD** - 実装前にテストを書く
4. **80%カバレッジ** 最低限
5. **多くの小さいファイル** - 200-400行が典型的、最大800行
6. **console.logなし** 本番コードに使用しない
7. **適切なエラーハンドリング** try/catchを使用
8. **入力バリデーション** Pydantic/Zodを使用
