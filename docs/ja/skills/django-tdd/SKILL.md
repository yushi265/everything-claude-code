---
name: django-tdd
description: pytest-djangoを使用したDjangoテスト戦略、TDD方法論、factory_boy、モッキング、カバレッジ、Django REST Framework APIのテスト。
---
...
- Djangoモデル、ビュー、シリアライザのテスト
- Djangoプロジェクトのテストインフラのセットアップ
...
- **Factoryを使用**: 手動でのオブジェクト作成の代わりに
- **1テストに1つのアサーション**: テストを集中させる
- **わかりやすいテスト名**: `test_user_cannot_delete_others_post`
- **エッジケースのテスト**: 空の入力、None値、境界条件
- **外部サービスのモック**: 外部APIに依存しない
- **Fixtureの使用**: 重複を排除
- **パーミッションのテスト**: 認可が機能することを確認
- **テストを高速に保つ**: `--reuse-db`と`--nomigrations`を使用

### すべきでないこと

- **Djangoの内部をテストしない**: Djangoが動作することを信頼する
- **サードパーティコードをテストしない**: ライブラリが動作することを信頼する
- **失敗テストを無視しない**: すべてのテストが合格しなければならない
- **テストを依存させない**: テストは任意の順序で実行できるべき
- **モックを過度に使用しない**: 外部依存のみをモック
- **プライベートメソッドをテストしない**: 公開インターフェースをテスト
...
| `assertRedirects` | リダイレクトを確認 |
| `assertTemplateUsed` | テンプレート使用を確認 |
| `mail.outbox` | 送信されたメールを確認 |

注意: テストはドキュメントです。良いテストはコードがどのように動作すべきかを説明します。シンプルで読みやすく、保守しやすく保ってください。
